    ...
    preferences {
        ...
        section() {
            input "gpio14State", title: "External Switch Value", "enum", multiple: false, required: false, options: ["HIGH", "LOW"], defaultValue: "HIGH";
                ...
                section() {
            input "powerOffset", title: "PowerOffset (expected value/current value )", "decimal", multiple: false, required: false;
        }
    }
...
def currentPower = Double.parseDouble(power);
if (powerOffset && powerOffset!=0){
currentPower = powerOffset*currentPower;
debug ("Power value $currentPower, powerOffset = $powerOffset, from device $power")
}
